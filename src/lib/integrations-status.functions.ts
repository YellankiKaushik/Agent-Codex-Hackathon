...
import { DEFAULT_OPENAI_MODEL, DEFAULT_OPENROUTER_MODEL } from "./ai-gateway.server";
...
export const getIntegrationsStatus = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return {
    ...
    enkrypt: {
      ...
      fallback: "OpenAI/OpenRouter-based safety validator or local passthrough",
    },
    featherless: {
      ...featherless,
      primary: featherlessPrimary,
      fallback: "OpenAI/OpenRouter/local grounded fallback",
    },
    supabase,
    openai: {
