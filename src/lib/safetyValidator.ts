...
// falls back to an OpenAI/OpenRouter-based validator so demo mode still works.
...
export type SafetyProvider =
  "enkrypt" | "fallback-openai" | "fallback-openrouter" | "passthrough" | "unavailable";
...
function getFallbackSafetyGateway() {
  if (process.env.OPENAI_API_KEY) {
    return {
      provider: "fallback-openai" as const,
  ...
  if (process.env.OPENROUTER_API_KEY) {
    return {
      provider: "fallback-openrouter" as const,
...
export async function validateReport(
  ...
  const fallbackGateway = getFallbackSafetyGateway();
  if (!fallbackGateway) {
    return {
      status: "safe",
      provider: "passthrough",
      ...
      validated: true,
      issues: [],
      fallbackReason: "ENKRYPT_API_KEY is missing and OpenAI/OpenRouter fallback is unavailable.",
  ...
  try {
    ...
    return {
      ...
      provider: fallbackGateway.provider,
      note: [
