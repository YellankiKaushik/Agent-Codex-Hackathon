...
import { checkDiscoveryRateLimit, checkVigilanceRateLimit, rateLimitIdentity } from "./ratelimit";
...
type GatewayProvider = "openai" | "openrouter";
type ReasoningGateway = {
  gateway: ReturnType<typeof createOpenAIProvider>;
  provider: GatewayProvider;
  model: string;
  reasoningProvider: Extract<ReasoningProvider, "openai-fallback" | "openrouter-fallback">;
  safetyProvider: "fallback-openai" | "fallback-openrouter";
};
...
function getGateway(): ReasoningGateway | null {
  if (process.env.OPENAI_API_KEY) {
    return {
      gateway: createOpenAIProvider(process.env.OPENAI_API_KEY),
      provider: "openai",
      model: process.env.OPENAI_MODEL || DEFAULT_OPENAI_MODEL,
      reasoningProvider: "openai-fallback",
      safetyProvider: "fallback-openai",
    };
  }
  if (process.env.OPENROUTER_API_KEY) {
    return {
      gateway: createOpenRouterProvider(process.env.OPENROUTER_API_KEY),
      provider: "openrouter",
      model: process.env.OPENROUTER_MODEL || DEFAULT_OPENROUTER_MODEL,
      reasoningProvider: "openrouter-fallback",
      safetyProvider: "fallback-openrouter",
    };
  }
  return null;
}
...
function isCompleteSchemeReport(text: string, schemes: Scheme[]) {
  ...
  const hasSource =
    /source\s*:/i.test(text) || schemes.some((scheme) => text.includes(scheme.sourceUrl));
...
export const extractProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ profile: CitizenProfile; followUp: string | null }> => {
    ...
    const trace = startTrace("agent.profile", {
      ...
      provider: gateway?.provider ?? "local-demo",
    });
    if (!gateway) {
    ...
    try {
      ...
      const generation = await generateText({
        model: gateway.gateway(gateway.model),
      ...
      await trace.end(
        { provider: gateway.provider, fallback: NoObjectGeneratedError.isInstance(e) },
        e,
      );
      await flushObservability();
  ...
  .handler(async ({ data }): Promise<DiscoveryReport> => {
    ...
    try {
      ...
      if (eligibleResults.length === 0) {
        ...
        if (!reportMarkdown) {
          reasoningAttemptedProviders.push(gateway ? gateway.reasoningProvider : "local-fallback");
          ...
          if (!gateway) {
            ...
            fallbackReason =
              fallbackReason ?? "No OpenAI/OpenRouter key configured; used local grounded report.";
            ...
            try {
              ...
              if (isCompleteSchemeReport(reportMarkdown, eligibleSchemes)) {
                reasoningProvider = gateway.reasoningProvider;
              } else {
                reportMarkdown = generateLocalReport(profile, eligibleSchemes, eligibleResults);
                reasoningProvider = "local-fallback";
                fallbackReason =
                  fallbackReason ??
                  `${gateway.provider === "openai" ? "OpenAI" : "OpenRouter"} output was incomplete; used local grounded report.`;
              }
              reportSpan.end({
                schemes: eligibleResults.length,
              ...
              fallbackReason =
                fallbackReason ??
                `${gateway.provider === "openai" ? "OpenAI" : "OpenRouter"} fallback failed; used local grounded report.`;
              reportSpan.end({
                schemes: eligibleResults.length,
                provider: "local-fallback",
                fallbackProviderFailed: gateway.provider,
      ...
      const safetySpan = trace.span("Enkrypt validation", {
        ...
        fallbackProvider: getGateway()?.safetyProvider ?? "passthrough",
      ...
      try {
        ...
        for (const scheme of candidates.slice(0, 5)) {
          ...
          if (r.confidence === "high" || r.confidence === "medium") {
            ...
            const fallbackGateway = getGateway();
            const alertReasoningSpan = trace.generation("Alert reasoning", {
              schemeId: scheme.id,
              primaryProvider: "featherless",
              fallbackProvider: fallbackGateway?.provider ?? "local-fallback",
            });
            ...
            if (featherlessReason.ok && featherlessReason.text.length >= 12) {
              ...
              if (fallbackGateway) {
                try {
                  const generation = await generateText({
                    model: fallbackGateway.gateway(fallbackGateway.model),
                    system:
...
Write the alert reason.`,
                  ...
                  if (generation.text.trim().length >= 12) {
                    ...
                    alertReasoningProvider = fallbackGateway.reasoningProvider;
                  }
                  alertReasoningSpan.end({
                    provider: alertReasoningProvider,
                    model: fallbackGateway.model,
                  ...
                  alertReasoningSpan.end({
                    provider: "local-fallback",
                    featherlessFallbackReason: featherlessReason.error,
                    fallbackProviderFailed: fallbackGateway.provider,
                    error: (e as Error).message?.slice(0, 160),
                  });
                }
              } else {
